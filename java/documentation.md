...
The Crate.IO JDBC driver is hosted on [Bintray Repository](https://bintray.com/crate/crate/crate-jdbc/view) and available via [JCenter](https://bintray.com/bintray/jcenter).
...
The ``executeUpdate()`` method executes the SQL statement in ``PreparedStatement`` object which must be an
...
The resulting array contains one element for each command in the batch. The elements of the array are set in order in which the statements were added to the batch.
...
to be used as the id of the new blob. In the app we decode a _Base64_ encoded string
which contains blob into a newly allocated byte array and compute its _sha1_ digest:
